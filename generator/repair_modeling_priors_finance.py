...
def main() -> None:
    parser = argparse.ArgumentParser(description="Repair only finance-related modeling priors sections in place")
    ...
    if current_mode(args.mode) != "research":
        raise RuntimeError("finance-priors repair helper is intended for research mode only")
    ...
    if last_error is not None:
        for attempt in range(4):
            completion = _generate_json_artifact(
                client=client,
                artifact_name=f"repair_modeling_priors_finance_attempt_{attempt + 1}",
                prompt=_modeling_priors_completion_prompt(args, "finance_repair", sections, last_error, working),
                model=model,
                temperature=0.0,
    ...
    print(f"Repaired finance-related modeling priors in {path}")
